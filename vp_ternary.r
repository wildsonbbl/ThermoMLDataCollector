...
tmlframe <- tmlframe %>%
  mutate(
    ...
    mass_fraction_c3p2 = case_when(
      ...
      !is.na(`Solvent: Mass ratio of component to other component of binary solvent c3 phase_2`) &
        !is.na(`Mass fraction c2 phase_2`) ~
        `Solvent: Mass ratio of component to other component of binary solvent c3 phase_2` /
          (1 + `Solvent: Mass ratio of component to other component of binary solvent c3 phase_2`) * (1 - `Mass fraction c2 phase_2`),
      !is.na(`Solvent: Mass ratio of component to other component of binary solvent c3 phase_2`) &
        !is.na(`Mass fraction c1 phase_2`) ~
        `Solvent: Mass ratio of component to other component of binary solvent c3 phase_2` /
          (1 + `Solvent: Mass ratio of component to other component of binary solvent c3 phase_2`) * (1 - `Mass fraction c1 phase_2`),
    )
  ) %>%
  mutate(
    mass_fraction_c1p2 = case_when(
...
tmlframe <- tmlframe %>%
  mutate(
    mole_fraction_c1p2 = case_when(
      ...
      !is.na(`Amount ratio of solute to solvent c1 phase_2`) & !is.na(`Mole fraction c2 phase_2`) ~ `Amount ratio of solute to solvent c1 phase_2` / (1 + `Amount ratio of solute to solvent c1 phase_2`) * (1 - `Mole fraction c2 phase_2`),
      !is.na(`Amount ratio of solute to solvent c1 phase_2`) & !is.na(`Mole fraction c3 phase_2`) ~ `Amount ratio of solute to solvent c1 phase_2` / (1 + `Amount ratio of solute to solvent c1 phase_2`) * (1 - `Mole fraction c3 phase_2`),
...
tml_saved <- read_parquet("vp_ternary.parquet")
tml_saved %>% colnames()
tml_saved %>% summary()
